...
def fetch_and_store_dart_outstanding_shares(db_path: Path, corp_code: str, stock_code: str, bsns_year: int, reprt_code: str, dart_api_key: str):
    logging.debug(f"Attempting to fetch outstanding shares for {corp_code} (stock_code: {stock_code}, bsns_year: {bsns_year}, reprt_code: {reprt_code}) from DART API...")
    ...
    try:
        ...
        if common_outstanding_shares > 0:
            with _get_db_connection(db_path) as conn:
                ...
                conn.commit()
            logging.debug(f"Successfully stored DART outstanding shares for {corp_code} (stock_code: {stock_code}, trade_date: {trade_date}, shares: {common_outstanding_shares}).")
        else:
